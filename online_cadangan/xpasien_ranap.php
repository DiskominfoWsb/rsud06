...
$(document).ready(function(){	
	//$("#tampil_data").load('tampil-pasien-ranap-rsud-krt-setjonegoro-wonosobo.html');	
	var combodokter = $("#combodokter").val();		
	ftampil_pasien_ranap(combodokter);
	$("#combodokter").change(function(){													
		var combodokter = $("#combodokter").val();		
		//alert('cek '+combodokter);
		ftampil_pasien_ranap(combodokter);
	});	
	function ftampil_pasien_ranap(x){			
		var kode_dokter = x;				
		$.ajax({
			type 	: "POST",
			url		: "tampil-pasien-ranap-rsud-krt-setjonegoro-wonosobo.html",
	...
	<div class="container">
			...
			<div class="login">				
				<?php 
						...
						$uri="https://simrsud.wonosobokab.go.id/webservicesms/pasien/service/combo_dokter/";	
							$datap = "123";
							$secretKey = "123";
				...
				<input type="hidden" name="nik" id="nik" value="<?php echo"$_SESSION[xnik]";?>" class="form-control" placeholder="NIK" />
					...
					<?php if($_SESSION['xlevel']=="admin"){?>
					<div class="input-group col-sm-2">
					<select class="form-control" id="combodokter">					
						<?php						
							if($data['status']=="200"){						
								foreach ($resultarr as $name => $xdata){											
									foreach ($xdata as $name1 => $value1){										
										echo"<option value='$value1[id_dokter_dms]'>$value1[dr_nm]</option>";
									}
								}		
							}else{
					...
					<?php }else{
						echo"<select class='form-control' id='combodokter'>
								<option value='$_SESSION[xsub_user_dokter_dms]'>$_SESSION[xnamauser]</option>
							</select>";
					}?>	
