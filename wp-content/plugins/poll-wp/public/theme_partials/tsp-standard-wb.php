 ...
 *
 * This file include standard and standard without button poll theme proporties.
 *
 * @link       TS Poll
 * @since      1.7.0
 *
...
for ( $i = 0;$i < $ts_poll_answers_count;$i ++ ) {
    ...
    <div class='ts_poll_answer' data-tsp-id='%s'>
      <input type='%s' class='ts_poll_answer_input'
            id='%s'
            name='%s'
            value='%s'>
        %s
      <span class='ts_poll_r_block'>
		...
		esc_attr( $t_s_poll_answers_values[ $i ]["id"] ),
		sprintf( "<label class='ts_poll_answer_label ts_poll_fa' for='%s'>%s</label>", 'ts_poll_answer_input_' . esc_attr( $total_soft_poll ) . '-' . esc_attr( $i + 1 ), esc_html( html_entity_decode( htmlspecialchars_decode( $t_s_poll_answers_values[ $i ]["Answer_Title"] ), ENT_QUOTES ) ) ),
